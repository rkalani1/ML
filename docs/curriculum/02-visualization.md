...
Modern visualization inherits three intertwined purposes. First, exploration: surface data quality problems, distributional shape, and subgroup structure before any model is trained. Second, diagnosis: residual plots, calibration diagrams, and confusion-matrix heatmaps can expose model failure modes that a single AUC hides. Third, communication: figures that a stroke attending, a data manager, and a methods reviewer can interrogate in the same meeting. For a neurologist–epidemiologist, the stakes are concrete. A skewed NIH Stroke Scale distribution with a pile of zeros may reflect a floor effect, coding process, selection mechanism, or multiple clinical regimes; it can violate the assumptions of a simple mean model and deserves investigation. A scatter of age against door-to-needle time colored by hospital site can reveal a process problem long before an accuracy number does.
...
Overall accuracy is the diagonal over the total: (30 + 35 + 30) / 130 = 95/130 ≈ 0.731. Row-normalizing gives recall relative to the chosen reference labels: LAA 30/40 = 0.75, CE 35/50 = 0.70, SVO 30/40 = 0.75. Column-normalizing gives precision against that reference: LAA 30/44 ≈ 0.68, CE 35/47 ≈ 0.74, SVO 30/39 ≈ 0.77. Thus roughly a third of predicted “LAA” labels disagree with the reference standard; this does not establish the patient’s true etiology when the reference itself is uncertain. State the normalization, reference-label process, and whether counts come from training, validation, or external assessment.

Reliability diagrams (calibration plots) bin predicted probabilities and compare mean predicted risk to observed event frequency within each bin. Perfect calibration lies on the diagonal. A model can have excellent discrimination (high AUROC) and still be systematically overconfident or underconfident after transport to a new site. Learning curves plot training and validation scores against training-set size and can suggest underfitting or variance-related limitations, but the patterns are heuristics rather than unique diagnoses. Slice-wise metrics can support equity and transportability audits provided subgroup definitions, denominators, uncertainty, and multiplicity are reported.
