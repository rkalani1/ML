...
![PCA: cumulative variance explained and reconstruction error vs k.](../assets/figures/ml_fig_pca_variance_recon.png)
...
LLE assumes each point lies on a locally linear patch of a manifold. For each point \(x_i\), find k nearest neighbors and reconstruct \(x_i\) as a weighted combination of those neighbors, minimizing reconstruction error with weights summing to one. Then find low-dimensional coordinates \(y_i\) that respect the same weights, minimizing \(\sum_i \lVert y_i - \sum_j W_{ij} y_j \rVert^2\) under centering and unit-covariance constraints. LLE can unroll smooth manifolds but is sensitive to k, noise, and gaps in sampling—common issues in small clinical cohorts.
...
Because t-SNE and UMAP optimize neighborhood-based objectives rather than globally preserving the original metric, a handful of specific misreadings recur in imaging and omics work, each with a clinical failure mode. Cluster footprint is not quantitatively preserved: t-SNE uses adaptive local bandwidths, while UMAP builds locally scaled fuzzy relationships and exposes layout controls such as `min_dist`; either method can expand or contract regions in ways that depend on its own objective and settings. Never read a blob’s visual spread as a calibrated estimate of biological heterogeneity or variance. Between-cluster distance is not globally calibrated: some local-neighbor relations may be retained, but distances and gaps between separated blobs do not provide a reliable quantitative scale for ranking subtype dissimilarity. Map density is likewise not a calibrated estimate of density in the original feature space. Most dangerous clinically, these methods can tear a genuine continuum into apparently discrete islands, or merge distinct groups into one—a smooth severity gradient (rising infarct volume, a titrated biomarker) can render as several disconnected clusters, inviting a false claim of discrete stroke subtypes when the biology is a continuum.
