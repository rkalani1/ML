...
## Opening
![Pretext pretrain then clinical fine-tune.](../assets/figures/swarm3h_ssl_pipeline.png)

*Pretext pretrain then clinical fine-tune.*
...
The exact RBM log-likelihood gradient is a data-driven positive phase minus a model-distribution negative phase. The partition function makes the negative-phase expectation intractable. Contrastive divergence CD-k replaces that expectation with samples from a short Gibbs chain initialized at the data, yielding a biased but often useful approximation; the model term does not simply cancel.
...
![Triplet loss with chapter worked numbers.](../assets/figures/ml_fig_triplet_ssl.png)
