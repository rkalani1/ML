...
Under MCAR, the probability of missingness depends on neither observed nor unobserved values. Under the usual sampling and model assumptions, complete-case estimates may remain unbiased but lose efficiency. Deterministic mean or median imputation still creates an artificial point mass, shrinks variance, and distorts associations. Observed-data tests can reject some MCAR patterns under assumptions, but they cannot prove that the missingness mechanism is MCAR.

### Missing At Random (MAR)

Under MAR, missingness depends only on observed variables, not on the unseen value itself once those observables are conditioned upon. Intuition: NIHSS is more often blank for rapidly improving TIA patients triaged as low-acuity—the blank is explained by observed acuity and care setting, not by the particular NIHSS score that was never recorded. MAR can support likelihood-based or multiple-imputation analyses, but it is not sufficient by itself: the imputation and analysis models must be appropriately specified and compatible, and variables that predict missingness or the missing value should be included. The assumption remains untestable from the observed data alone.

### Missing Not At Random (MNAR)

Under MNAR, missingness depends on the unobserved value itself even after conditioning on observed variables. For example, a patient may decline a cognitive item because of impairment severity not captured elsewhere, so nonresponse still depends on the unseen score after measured covariates are considered. By contrast, a troponin omitted because recorded clinical appearance suggested low risk is MAR conditional on that appearance. MNAR analyses require explicit, unverifiable assumptions—often explored with selection models, pattern-mixture models, or sensitivity analyses—rather than a single fill value treated as truth.

### Translating Mechanism into Features

The analysis goal determines the response. For prediction, fit imputation entirely within training folds, consider a missingness indicator only when that signal will exist at deployment, and evaluate under realistic deployment missingness. For inference, use methods such as multiple imputation or likelihood-based modeling that propagate uncertainty under explicit assumptions. Do not “impute and forget,” and do not assume that a care-process proxy will remain stable across sites or eras.

It is also where leakage hides. A “troponin missing” flag is a lawful index-time feature only if the ordering decision and its resolution are settled by t₀; if troponin typically results hours after the ED disposition being modeled, then “troponin measured” smuggles the future workup into a t₀ model. Worse, a blank created by the outcome—a discharge field empty because the patient died before discharge coding—encodes the label as absence, the purest leakage disguised as informative missingness. Two disciplines follow. First, fit every imputer on the training fold only: imputing with the full-cohort mean leaks the test distribution into each row, the same error as fitting a scaler on all data. Second, certify each missingness indicator against the timeline exactly as you would any other feature. The handling method must match the estimand, analysis model, and plausible mechanism; MAR-based imputation includes missingness drivers, while MNAR calls for explicit sensitivity analysis. No mechanism makes a future or post-outcome field legal at index time.
...
(11) Classify the first two blanks and distinguish the third: (a) glucose panels voided one afternoon by a lab-analyzer outage; (b) troponin never ordered because recorded clinical appearance suggested low risk; (c) 90-day mRS whose follow-up window has not yet elapsed. Item (c) is pending, not yet classifiable as MCAR/MAR/MNAR; at ED arrival it is a future target and never a feature. For each item, state whether any missingness indicator is legal at the claimed prediction time.
