...
K-means minimizes within-cluster sum of squares J = Σ_i Σ_j z_{ij} ||x_i − μ_j||² with hard assignments z_{ij} ∈ {0,1}. Lloyd’s algorithm: (1) initialize centers; (2) assign each point to the nearest center; (3) replace each center by the mean of its assigned points; (4) repeat until stable. The objective does not increase during Lloyd updates, but the result can depend on initialization and need not be globally optimal. K-means++ and multiple restarts are common safeguards, with stability reported rather than assumed. Assumptions: roughly spherical, similar-sized clusters in Euclidean space; sensitivity to outliers because of squares.
...
K-medians replaces squared Euclidean loss with coordinate-wise L1 distance and updates centers to coordinate-wise medians in \(\mathbb{R}^p\), making it less sensitive than squared loss to extreme coordinate values. K-medoids requires centers to be actual data points (medoids), minimizing total dissimilarity to medoids; PAM is a classic algorithm. Medoids can work with arbitrary dissimilarity matrices, including DTW or Jaccard—unlike means, which require vector averaging.
...
D, E, F = (6, 5), (7, 6), (6, 7)
mu1, mu2 = (1, 1), (7, 6)
# Iteration 1 assigns {A,B,C} -> 1 and {D,E,F} -> 2.
# Updated means: mu1=(4/3,4/3), mu2=(19/3,6).
# Iteration 2 leaves assignments unchanged, so J=4 and the algorithm stops.
```
...
![Elbow plot of WSS vs k on the six-point toy set.](../assets/figures/ml_fig_elbow_wss.png)
...
![Mean silhouette vs k on synthetic three-blob data.](../assets/figures/ml_fig_silhouette_k.png)
...
When n is large and memory is tight, BIRCH-style summarization or mini-batch k-means become practical; CURE-style multiple representatives help when shapes are non-spherical but still compact enough to summarize. When uncertainty of assignment matters and elliptical components are plausible, GMMs with EM provide soft responsibilities and a likelihood for model comparison (AIC/BIC with caution). Fuzzy C-means offers soft memberships without a full generative story, but remains sensitive to outliers and initialization; memberships must not be read as calibrated disease probabilities.
...
# C1={A,B,C}, C2={D,E,F}; a(A)=1 and b(A)~7.341.
a_A, b_A = 1.0, 7.341
s_A = (b_A - a_A) / max(a_A, b_A)
print(round(s_A, 3))  # 0.864
```
...
Stroke research has long used subtype taxonomies built from expert rules, imaging, and vascular workups. Unsupervised clustering returns structure relative to features, distance, and algorithm. Imaging-derived clusters can recover scanner or site signatures rather than biology. Candidate responses include harmonization fit without assessment-set leakage, scanner-stratified analyses, and scanner- or site-held-out replication. ComBat-style adjustment can also remove real biological signal when site and biology are entangled, so no preprocessing method by itself establishes a biological phenotype.
...
Suppose a one-dimensional two-component mixture with equal weights π_1 = π_2 = 0.5, means μ_1 = 0, μ_2 = 6, and shared variance σ² = 1. For a point x = 2, the Gaussian densities are proportional to exp(−(2−0)²/2) = exp(−2) ≈ 0.1353 and exp(−(2−6)²/2) = exp(−8) ≈ 0.000335. Responsibilities: r_1 ∝ 0.5·0.1353, r_2 ∝ 0.5·0.000335, so after normalization r_1 ≈ 0.9975, r_2 ≈ 0.0025. The point is softly but almost certainly in component 1. For x = 3, the midpoint of the two means, both densities equal exp(−(3)²/2) = exp(−(3−6)²/2) = exp(−4.5) ≈ 0.0111, so r ≈ (0.5, 0.5). Boundary patients get honest uncertainty; hard k-means would still force a single label.
