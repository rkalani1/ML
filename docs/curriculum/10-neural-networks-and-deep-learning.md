...
Figure 10.1. A multilayer perceptron with three input units, one hidden layer of four ReLU units, and two output units, fully connected between adjacent layers. Each hidden or output unit computes h^(l) = phi(W^(l) h^(l-1) + b^(l)); stacking these affine-then-nonlinear maps is what lets depth build hierarchical features that a single linear map cannot.
