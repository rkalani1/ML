...
This expanded edition is designed so that studying this document alone delivers the topical mastery a careful reader would gain from the full Rawassizadeh curriculum: foundations, unsupervised learning, feature engineering and decomposition, supervised methods, deep and self-supervised learning, multimodal models, reinforcement learning, efficient models, graph mining, and data challenges. Exposition is original; the syllabus is comprehensive.
...
Work numerical examples by hand; recompute intermediates.

Map each method to prediction vs etiology vs decision support.

There is no separate further-reading chapter—the book is the curriculum.
