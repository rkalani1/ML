...
Concept drift. A change over time in \(P(Y\mid X)\), the relationship between features and the target. Changes only in \(P(X)\) or \(P(Y)\) are covariate shift or label shift; all three can degrade a deployed model and require monitoring.
