...
Squared error loss ℓ = (y − ŷ)² underpins classical linear regression and is sensitive to outliers. Absolute error |y − ŷ| is more robust. Huber loss is quadratic for small residuals and linear for large residuals, reducing the influence of large vertical residuals while remaining smooth near zero. It does not by itself protect against observations with extreme predictor leverage. For probabilistic binary classification, log loss is the negative Bernoulli log-likelihood; choosing a loss determines which errors the model emphasizes.
...
![OLS fit for the four-point NIHSS–volume example.](../assets/figures/ml_fig_ols_fit.png)

*Exact OLS line ŷ = 66/13 + (23/13)x with residual segments; RSS ≈ 1.23, R² ≈ 0.992.*
...
![Gradient noise: batch GD versus mini-batch SGD paths on a synthetic elongated quadratic.](../assets/figures/ml_fig_gradient_noise.png)

*Figure — Gradient noise. Full-batch steps follow a smooth trajectory; under approximately independent sampling, mini-batch gradient variance scales roughly as 1/batch size, while its standard deviation or noise amplitude scales as 1/√batch size. That noise can slow exact convergence while sometimes helping exploration in nonconvex objectives.*
...
![Early stopping: validation loss bottoms while training loss continues to fall.](../assets/figures/ml_fig_early_stopping.png)
