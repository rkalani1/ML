...
Training: minimize empirical risk with optional regularization on training data; use validation data for tuning without fitting on the final test set.
...
Rules are attractive when interpretability and auditability dominate. A synthetic non-care example—IF required research fields are complete THEN route the record for retrospective audit—is a human-authored rule classifier. Real stroke imaging or treatment pathways must come from current, locally approved protocols and accountable clinical governance; this chapter does not supply eligibility rules. Learned rules can approximate tree leaves flattened into logical form, but remain brittle at continuous boundaries and can be expensive to search.
...
Normalizing by their sum—this sum is the evidence term p(x) in Bayes theorem—gives the posterior P(L+ | x) = 0.224 / (0.224 + 0.024) = 0.224 / 0.248 ≈ 0.903, and P(L− | x) ≈ 0.097. Under this deliberately simplified toy model and its naive conditional-independence assumption, a patient with both signs receives roughly 90% modeled probability of LVO; this is not an empirically calibrated bedside estimate. An odds-form cross-check verifies the arithmetic: prior odds are 0.4/0.6 = 0.667, the likelihood ratios are 0.8/0.2 = 4 for gaze and 0.7/0.2 = 3.5 for arm, so posterior odds = 0.667 · 4 · 3.5 = 9.33 and posterior probability = 9.33 / (1 + 9.33) ≈ 0.903—identical to the normalized computation.
...
Random forests bag many deep trees and consider a random subset of features at each split. Feature importance is heuristic: impurity-decrease importance tends to favor continuous or high-cardinality predictors, while permutation importance is model- and data-dependent and can be diluted among correlated predictors. Out-of-bag estimates are useful, but an untouched assessment set is still needed for final claims.
...
XGBoost, LightGBM, and CatBoost share additive-tree foundations but differ in objectives, categorical handling, tree growth, systems choices, and defaults. Their speed, memory, overfitting, and accuracy rankings depend on the data, hardware, constraints, and tuning protocol. Benchmark candidates under the same leakage-controlled splits and tuning budget rather than assuming a universal winner.
...
![Threshold trade-off: overlapping scores and sensitivity/specificity vs t.](../assets/figures/ml_fig_threshold_sens_spec.png)

*Figure — Synthetic threshold trade-off. The Youden peak is one mathematical criterion, not a clinical authorization threshold. No threshold in this teaching example should trigger angiography, transfer, treatment, or withholding; any consequential operating point requires task-specific evidence, current protocols, human-factors testing, and institutional and regulatory governance.*

ROC curves plot true positive rate versus false positive rate as the threshold varies. AUC-ROC summarizes ranking quality. Precision–recall curves are often more informative when positives are rare. Reliability diagrams and ECE summarize aspects of calibration but are bin- and sample-dependent. Platt scaling and isotonic regression are candidate recalibration methods that must be fit and evaluated on separated data.

![Precision–recall and ROC for the same synthetic imbalanced cohort.](../assets/figures/ml_fig_precision_recall.png)
...
Class weighting or resampling changes the effective training distribution or loss and can distort probability calibration. Evaluate probabilities on data reflecting the intended setting; any prior correction or recalibration requires explicit assumptions and separate assessment. No risk-score threshold should independently withhold time-sensitive reperfusion therapy. A candidate threshold for clinical use requires prospective evidence, current guidelines and local protocols, human oversight, and applicable institutional and regulatory review.
