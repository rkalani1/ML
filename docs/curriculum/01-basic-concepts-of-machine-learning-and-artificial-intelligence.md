...
Streams are unbounded or continuously arriving sequences where storing everything forever is impossible or undesirable: bedside monitors, clickstreams, continuous EEG alerts, hospital ADT feeds. Stream learning emphasizes constant-time or logarithmic updates, concept drift detection, and sliding or fading windows. Evaluation must respect time order; random shuffles can destroy the temporal structure of the evaluation and leak future observations into training. Operational metrics include false alarms per hour and time-to-detection, not only offline AUC.
...
![Training vs validation error versus model capacity.](../assets/figures/ml_fig_bias_capacity.png)
...
# If false negatives cost twice as much as false positives, compare costs too.
```
...
![Bias–variance decomposition of expected squared error versus model capacity.](../assets/figures/ml_fig_bias_variance_decomp.png)
...
Fairness concerns systematic differences in a model’s behavior across groups defined by attributes such as age, sex, race or ethnicity, language, insurance status, or geography. Two mechanisms can produce unfairness even with well-intentioned engineering. First, label bias: if the ground truth itself reflects unequal access or documentation—say, under-coding of stroke in populations with poorer access to imaging—then a model trained to reproduce those labels can inherit and amplify the disparity. Second, representation bias: groups scarce in the training data may receive higher-variance or poorly calibrated predictions. Fairness should therefore be evaluated with decision-relevant, disaggregated performance and uncertainty—not by a single pooled number that can hide subgroup harm. With imperfect prediction and differing base rates, several intuitive criteria such as equal error rates and equal calibration generally cannot all hold simultaneously, so the appropriate goals require an explicit, legally and ethically reviewed choice tied to the decision and its harms.

Privacy is a first-class constraint because health data are identifying and sensitive. De-identification reduces but does not eliminate re-identification risk, especially with rich longitudinal or imaging data. High-capacity models can memorize training examples, exposing them to membership-inference attacks (an adversary infers whether a specific patient was in the training set) or, for generative models, verbatim regurgitation. A correctly implemented differential-privacy mechanism bounds an explicitly defined privacy loss under its stated adjacency relation, \(\varepsilon\), \(\delta\), and composition accounting; the guarantee does not extend beyond those assumptions. Federated learning can keep raw records at participating sites, but model updates, metadata, or a compromised participant can still leak information. Neither technique replaces access control, security, data-use authority, or privacy review.
