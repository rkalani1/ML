...
Figure 16.4. Monitoring for dataset shift across deployment windows. (a) Live AUROC is tracked monthly; concept drift (a change in P(Y|X)) erodes performance until it breaches the minimum-AUROC threshold of 0.80 at month 8 (circled), triggering a governed retraining that restores discrimination (emerald segment). (b) Covariate drift in an input feature: the current-window admission-glucose distribution (amber) has moved right of the reference (indigo). Binning the reference into deciles gives a population stability index PSI = Σᵢ (cᵢ − rᵢ)·ln(cᵢ / rᵢ) = 0.42, well above the ≈0.2 alarm level and flagging a material shift. Re-estimate calibration and PPV at the new distribution rather than auto-retraining without change control.
