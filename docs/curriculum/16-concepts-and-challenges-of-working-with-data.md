...
!!! note "Figure concept (text diagram) 16.1"

    Sampling a target distribution with Markov Chain Monte Carlo. Left: a Metropolis random-walk chain for a standard-normal target N(0,1) uses a symmetric proposal θ' = θ + N(0, 2.4²) and accepts with probability min(1, exp[−½(θ'²−θ²)]), so the proposal density cancels; deliberately started in the tail at θ = 6, the chain forgets its origin during the shaded burn-in (first 1000 steps, discarded) and then explores around the target mean 0 at a ~44% acceptance rate. Right: the histogram of the post-burn-in draws (mean ≈ 0.02, sd ≈ 1.03) reproduces the true N(0,1) density (rose curve), illustrating how dependent MCMC samples approximate posterior expectations after burn-in.
...
!!! note "Figure concept (text diagram) 16.2"

    Correcting class imbalance with SMOTE-style oversampling. Left: a majority class (260 points, indigo) surrounds a rare minority cluster (18 seeds, rose); SMOTE synthesizes new minority examples by interpolating along the segments to nearest minority neighbours, x_new = x_i + λ(x_nn − x_i) with λ ~ U(0,1), so each hollow point lies on a real neighbour segment rather than as arbitrary noise. Right: class counts before and after oversampling — the majority is unchanged at 260 while the minority rises from 18 to 72 — showing how resampling rebalances the training set. Synthesize only within the training fold to avoid leakage, and judge the rare class with precision–recall, not accuracy.
...
!!! note "Figure concept (text diagram) 16.3"

    Missingness mechanisms and imputation in a patient × feature matrix. (a) Cells are shaded by value; dashed rose cells marked '?' are missing. Three columns illustrate Rubin's mechanisms: Glucose is missing completely at random (MCAR), NIHSS is missing at random (MAR) — blank exactly when the observed stroke-code is off — and 90-day mRS is missing not at random (MNAR), lost precisely for the most disabled patients. (b) The same matrix after column-mean imputation, with amber-dashed cells marking the filled values. Mean-fill is only a baseline that distorts variance and correlation; kNN, regression/MICE, or multiple imputation preserve structure and propagate uncertainty, and the outcome label must never be imputed or leaked.
...
!!! note "Figure concept (text diagram) 16.4"

    Monitoring for dataset shift across deployment windows. (a) Live AUROC is tracked monthly; concept drift (a change in P(Y|X)) erodes performance until it breaches the minimum-AUROC threshold of 0.80 at month 8 (circled), triggering a governed retraining that restores discrimination (emerald segment). (b) Covariate drift in an input feature: the current-window admission-glucose distribution (amber) has moved right of the reference (indigo). Binning the reference into deciles gives a population stability index PSI = Σᵢ (cᵢ − rᵢ)·ln(cᵢ / rᵢ) = 0.42, well above the ≈0.2 alarm level and flagging a material shift. Re-estimate calibration and PPV at the new distribution rather than auto-retraining without change control.
...
!!! note "Figure concept (text diagram) 16.5"

    Cohen's κ adjusts rater agreement for chance. Two reviewers independently label n = 100 non-contrast head CTs for intracranial haemorrhage, giving the 2×2 table 40/10/5/45 (the diagonal cells, emerald-bordered, are agreements). Observed agreement is pₒ = (40 + 45)/100 = 0.85, but multiplying the raters' marginal proportions gives chance agreement pₑ = (50/100)(45/100) + (50/100)(55/100) = 0.225 + 0.275 = 0.50, so κ = (pₒ − pₑ)/(1 − pₑ) = 0.35/0.50 = 0.70 — 'substantial' on the Landis–Koch scale. The fall from 85% raw to 70% chance-adjusted agreement is a label-reliability ceiling; discrimination a model claims above it deserves suspicion.
...
!!! note "Figure concept (text diagram) 16.6"

    A retrieval-augmented generation (RAG) pipeline. Offline (bottom lane), source documents — guidelines and curated notes — are chunked, embedded, and stored in a vector database backed by an approximate-nearest-neighbour index (HNSW). Online (top lane), a clinician query is embedded, the retriever pulls the top-k matching chunks, and those chunks together with the original question form the prompt sent to the LLM, which returns a grounded answer with citations for human verification. Because knowledge lives in the index, it stays editable by re-indexing instead of re-training; the main failure modes are retrieval misses, contradictory or stale chunks, and prompt injection in retrieved text, so RAG is evaluated on answer faithfulness and retrieval recall@k, not chat fluency.
...
!!! note "Figure concept (text diagram) 16.7"

    A SHAP local explanation drawn as a waterfall. For one toy stroke case, the model's predicted probability of a poor 90-day outcome (mRS > 2) starts at the base rate E[f(x)] = 0.30, and each feature's Shapley value φᵢ pushes it up (indigo, increases risk) or down (rose, decreases risk): a high NIHSS of 18 (+0.22) and a hyperdense-vessel sign (+0.10) raise the risk, while successful reperfusion (TICI 3, −0.09) and a good ASPECTS of 9 (−0.07) lower it. By the efficiency axiom the attributions sum exactly to the output, base + Σᵢ φᵢ = 0.30 + 0.20 = 0.50 = f(x). SHAP explains the model, not causation — its strongest clinical use is detecting label leakage from post-outcome features.
