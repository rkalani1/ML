#!/usr/bin/env python3
"""Cycle-233/234 quality densify: novel scientific teal teaching panels."""
...
C233 = [
    ("QR iteration residual decay", make_curve("exp")),
    ("Privacy budget composition sum", make_curve("log")),
    ("Natarajan dimension growth", make_curve("power")),
    ("Small-multiple correlation heat", make_heat()),
    ("Gelman-Rubin R-hat approach", make_curve("sigmoid")),
    ("Bandwidth multi-scale clusters", make_scatter()),
    ("GSP join-prune support path", make_flow(["scan", "join", "prune", "F_k"])),
    ("Hash dim collision tradeoff", make_curve("power")),
    ("t-SNE early exaggeration cool", make_curve("damped")),
    ("LAD vs squared residual cost", make_curve("relu")),
    ("One-vs-rest score margins", make_bars()),
    ("Residual stream add path", make_flow(["x", "F(x)", "+", "out"])),
    ("BYOL predictor stop-grad", make_flow(["online", "pred", "target", "MSE"])),
    ("ViT patch token affinity", make_heat()),
    ("REINFORCE baseline variance", make_semilog()),
    ("INT8 scale and zero-point", make_bars()),
    ("Louvain modularity gain path", make_curve("sigmoid")),
    ("Point-in-time feature join", make_flow(["as-of", "join", "train", "serve"])),
    ("Error budget burn alert", make_curve("log")),
    ("Glossary loss family strip", make_flow(["CE", "hinge", "Huber", "focal"])),
]

C234 = [
    ("Conjugate gradient residual", make_curve("exp")),
    ("DPIA control sign-off path", make_flow(["need", "risk", "controls", "sign"])),
    ("Stability selection freqs", make_bars()),
    ("Raincloud class comparison", make_scatter()),
    ("NUTS phase-space field", make_quiver()),
    ("Spectral bicluster blocks", make_heat()),
    ("PrefixSpan project grow", make_flow(["SDB", "prefix", "project", "grow"])),
    ("Monotone WoE bin heights", make_bars()),
    ("NMF recon error vs rank", make_curve("power")),
    ("Pinball quantile loss shape", make_curve("relu")),
    ("Softmax temperature map", make_curve("sigmoid")),
    ("Activation checkpoint trade", make_curve("log")),
    ("MoCo queue contrast path", make_flow(["query", "queue", "keys", "NCE"])),
    ("ASR encoder-decoder path", make_flow(["audio", "enc", "dec", "text"])),
    ("PPO clip ratio objective", make_curve("damped")),
    ("Column-wise quant error", make_bars()),
    ("Soft label diffusion path", make_curve("sigmoid")),
    ("Schema break rate trend", make_curve("log")),
    ("Action item burn-down bars", make_bars()),
    ("Glossary SSL loss strip", make_flow(["NCE", "BYOL", "VICReg", "MAE"])),
]
...
if __name__ == "__main__":
    ...
    m = {233: C233, 234: C234}
    cycles = [int(x) for x in sys.argv[1].split(",")] if len(sys.argv) > 1 else [233, 234]
    for c in cycles:
        embed(c, m[c])
