#!/usr/bin/env python3
"""Cycle-237/238 quality densify: novel scientific teal teaching panels."""
...
C237 = [
    ("Power iteration residual decay", make_curve("exp")),
    ("Moments accountant epsilon path", make_curve("log")),
    ("Fat-shattering sample growth", make_curve("power")),
    ("Confusion structure heat map", make_heat()),
    ("ESS per chain path", make_curve("sigmoid")),
    ("Noise vs dense modes scatter", make_scatter()),
    ("Apriori level-wise path", make_flow(["scan", "join", "prune", "F_k"])),
    ("MinHash Jaccard estimate error", make_curve("power")),
    ("LargeVis negative sample cool", make_curve("damped")),
    ("Tukey biweight residual cost", make_curve("relu")),
    ("Top-k accuracy bars", make_bars()),
    ("Post-norm residual highway", make_flow(["x", "F(x)", "+", "out"])),
    ("Barlow cross-corr path", make_flow(["online", "pred", "target", "MSE"])),
    ("Relative position bias heat", make_heat()),
    ("GAE lambda return scale", make_semilog()),
    ("Group-wise bit allocation", make_bars()),
    ("Infomap map-equation drop", make_curve("sigmoid")),
    ("Online offline store path", make_flow(["as-of", "join", "train", "serve"])),
    ("SLO error budget path", make_curve("log")),
    ("Glossary calib metric strip", make_flow(["CE", "hinge", "Huber", "focal"])),
]

C238 = [
    ("BiCGSTAB residual path", make_curve("exp")),
    ("Red-team findings path", make_flow(["need", "risk", "controls", "sign"])),
    ("Permutation importance bars", make_bars()),
    ("One-class support cloud", make_scatter()),
    ("Langevin dynamics field", make_quiver()),
    ("Stochastic block heat", make_heat()),
    ("BIDE bi-directional path", make_flow(["SDB", "prefix", "project", "grow"])),
    ("Chi-square bin bars", make_bars()),
    ("PCA energy vs components", make_curve("power")),
    ("Poisson deviance residual", make_curve("relu")),
    ("Sparsemax projection map", make_curve("sigmoid")),
    ("Mixed-precision speed trade", make_curve("log")),
    ("SupCon supervised path", make_flow(["query", "queue", "keys", "NCE"])),
    ("RNN-T joiner path", make_flow(["audio", "enc", "dec", "text"])),
    ("DPO preference margin", make_curve("damped")),
    ("AWQ protected channel bars", make_bars()),
    ("Mixup lambda schedule", make_curve("sigmoid")),
    ("Freshness lag spike", make_curve("log")),
    ("Alert noise burn-down", make_bars()),
    ("Glossary graph algo strip", make_flow(["NCE", "BYOL", "VICReg", "MAE"])),
]
...
if __name__ == "__main__":
    ...
    m = {237: C237, 238: C238}
    cycles = [int(x) for x in sys.argv[1].split(",")] if len(sys.argv) > 1 else [237, 238]
    for c in cycles:
        embed(c, m[c])
